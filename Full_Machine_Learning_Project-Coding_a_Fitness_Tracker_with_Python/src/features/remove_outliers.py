...
sensors_df_columns = df_columns[0:6]

# Select numerical columns
outliers_columns = list(df.columns[0:6])

# Plot all columns separately
for item in sensors_df_columns:
    df[[f"{item}", "label"]].boxplot(by="label", figsize=(20, 10))

# Plot the accelerometer columns together
df[outliers_columns[:3] + ["label"]].boxplot(by="label", figsize=(20, 10), layout=(1, 3))

# Plot the gyroscope columns together
df[outliers_columns[3:] + ["label"]].boxplot(by="label", figsize=(20, 10), layout=(1, 3))
...
# Using Chauvenet
for col in outliers_columns:
    dataset = mark_outliers_chauvenet(df[df["label"] == label], col, C=2)
    plot_binary_outliers(dataset=dataset, col=col, outlier_col= col + "_outlier", reset_index=True)
...
outliers_removed_df.to_pickle(f"{real_path_to_interim}/02_outliers_removed_chauvenet.pkl")
