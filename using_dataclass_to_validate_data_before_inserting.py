...
with sqlite3.connect("contacts.db") as conn:
    ...
    if contact.validate():
        cur.execute("INSERT INTO contacts VALUES (?, ?, ?)", (contact.id, contact.name, contact.email))
        conn.commit()
    ...
    try:
        ...
        if bad_contact.validate():
            cur.execute("INSERT INTO contacts VALUES (?, ?, ?)", (bad_contact.id, bad_contact.name, bad_contact.email))
            conn.commit()
